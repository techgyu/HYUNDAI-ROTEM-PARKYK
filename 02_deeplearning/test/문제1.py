# --------------------------------------------------------------------------------------------------------------
# 문제1) https://www.kaggle.com/jyotikumarrout/graduation 의 binary.csv 
# 캐글 데이터셋을 활용한 대학원 입학 예측 모델 개발

# 데이터를 이용하여 미국 대학원 입학여부를 분류하는 모델을 작성하시오. loss, accuracy에 대한 시각화도 실시한다.
# GRE 점수, GPA, 학교 순위를 기반으로 입학 가능성을 예측하는 이진 분류 문제

# input 함수를 사용해 새로운 gre(점수), gpa(학점), rank(석차) 값을 받아  admit을 판정하시오.
# 사용자로부터 직접 입력받은 값으로 실시간 예측을 수행
# --------------------------------------------------------------------------------------------------------------
...
# 랜덤 시드 설정 - 실험 재현성을 위해 난수 생성기 초기화
np.random.seed(42)  # NumPy 난수 시드 고정
tf.random.set_seed(42)  # TensorFlow 난수 시드 고정

# 1. 데이터 로딩 및 분석
data = pd.read_csv('binary.csv')  # CSV 파일에서 대학원 입학 데이터 로드
print("=== 데이터 정보 ===")  # 데이터 정보 출력 헤더
print(data.info())  # 데이터프레임의 구조와 타입 정보 출력
print("\n=== 기본 통계 ===")  # 기본 통계 정보 출력 헤더
print(data.describe())  # 수치형 컬럼들의 기본 통계량 (평균, 표준편차 등) 출력
print("\n=== 클래스 분포 ===")  # 클래스 분포 출력 헤더
print(data['admit'].value_counts())  # 입학(1)과 비입학(0)의 개수 분포 확인

# 2. 입 출력 분리
x = data[['gre', 'gpa', 'rank']]  # 입력 특성: GRE 점수, GPA, 학교 순위
y = data['admit']  # 출력(타겟): 입학 여부 (0 또는 1)
...
# 6. Loss와 Accuracy 시각화
plt.figure(figsize=(15, 5))  # 전체 그래프 크기 설정 (가로 15, 세로 5)

# Sequential Model 시각화
plt.subplot(1, 3, 1)  # 1행 3열 중 첫 번째 서브플롯
plt.plot(history1.history['loss'], label='Training Loss')  # 훈련 손실 그래프
plt.plot(history1.history['val_loss'], label='Validation Loss')  # 검증 손실 그래프
plt.plot(history1.history['accuracy'], label='Training Accuracy')  # 훈련 정확도 그래프
plt.plot(history1.history['val_accuracy'], label='Validation Accuracy')  # 검증 정확도 그래프
plt.title('Sequential Model')  # 그래프 제목
...
plt.plot(history2.history['loss'], label='Training Loss')  # 훈련 손실 그래프
plt.plot(history2.history['val_loss'], label='Validation Loss')  # 검증 손실 그래프
plt.plot(history2.history['accuracy'], label='Training Accuracy')  # 훈련 정확도 그래프
plt.plot(history2.history['val_accuracy'], label='Validation Accuracy')  # 검증 정확도 그래프
plt.title('Functional API Model')  # 그래프 제목
...
plt.plot(history3.history['loss'], label='Training Loss')  # 훈련 손실 그래프
plt.plot(history3.history['val_loss'], label='Validation Loss')  # 검증 손실 그래프
plt.plot(history3.history['accuracy'], label='Training Accuracy')  # 훈련 정확도 그래프
plt.plot(history3.history['val_accuracy'], label='Validation Accuracy')  # 검증 정확도 그래프
plt.title('Subclassing API Model')  # 그래프 제목
plt.xlabel('Epochs')  # x축 라벨
plt.ylabel('Loss/Accuracy')  # y축 라벨
plt.legend()  # 범례 표시
plt.grid(True)  # 격자 표시
...
print("\n=== 대학원 입학 예측 ===")  # 사용자 입력 섹션 시작 메시지
try:
    # 사용자로부터 입학 지원 정보 입력받기
    gre_score = float(input("GRE 점수를 입력하세요 (200-800): "))  # GRE 점수 입력
    gpa_score = float(input("GPA를 입력하세요 (0.0-4.0): "))  # GPA 입력
    rank_score = int(input("학교 순위를 입력하세요 (1-4): "))  # 학교 순위 입력
    
    # 입력값을 배열로 변환하고 정규화 적용
    user_input = np.array([[gre_score, gpa_score, rank_score]])  # 2차원 배열로 변환
