...
model = Perceptron(max_iter=1000, eta0=0.1).fit(x_train, y_train)
...
def plot_decision_regions(X, y, classifier):
    ...
    plt.xlabel('꽃받침길이(sepal length) [cm]')

    plt.ylabel('꽃받침너비(sepal width) [cm]')
...
dtree = DecisionTreeClassifier(criterion='entropy', max_depth=1, random_state=0).fit(X, y)

plot_decision_regions(X, y, dtree)
#--------------------
#DecisionTree Classification - 의사 결정 나무 분류 모형의 사용 예: Iris
